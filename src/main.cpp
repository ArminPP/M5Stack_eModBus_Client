...
void loop()
{
  ...
  if (millis() - next_request > READ_INTERVAL)
  {
    ...
    if (BUTTON2)
    {
      ...
      // Serial.printf("\nRequested from Server2@ID%2i  Time: %8.3fs\n", SERVER2_ID, request_time2 / 1000.0);
      // M5.Lcd.setCursor(1, 100);
      // M5.Lcd.printf("Server2@ID%2i  Time:%8.3fs\n", SERVER2_ID, request_time2 / 1000.0);
    ...
    if (BUTTON3)
    {
      Serial.printf("Requested from Server3 @ID %2i\n", SERVER3_ID);
      M5.Lcd.setCursor(1, 120);
      M5.Lcd.printf("Server3@ ID %2i\n", SERVER3_ID);
      // Serial.printf("\nRequested from Server3@ID%2i  Time: %8.3fs\n", SERVER3_ID, request_time3 / 1000.0);
      // M5.Lcd.setCursor(1, 100);
      // M5.Lcd.printf("Server3@ID%2i  Time:%8.3fs\n", SERVER3_ID, request_time3 / 1000.0);
